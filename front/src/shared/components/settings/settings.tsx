export default function Settings() {
	return (
		<>
			...
			<aside id="overlay-custom-backdrop-2" className="overlay overlay-open:translate-x-0 drawer drawer-start hidden max-w-72 bg-[#17212b] text-white " tabIndex={-1}>
				{/* Верхняя часть с профилем */}
				<div className="p-4 flex flex-col gap-3">
					<div className="flex justify-between items-start">
						<div className="avatar">
							<div className="w-16 rounded-full">
								<img src="https://upload.wikimedia.org/wikipedia/commons/thumb/2/2d/TypeScript_ESLint_logo.svg/330px-TypeScript_ESLint_logo.svg.png" alt="avatar" />
							</div>
						</div>
						{/* Кнопка закрытия (опционально, в ТГ ее нет, но для UI полезна) */}
						<button type="button" className="btn btn-text btn-circle btn-sm text-white/50" data-overlay="#overlay-custom-backdrop-2">
							<span className="icon-[tabler--x] size-5"></span>
						</button>
					</div>
					...
					<div className="flex justify-between items-center group cursor-pointer">
						<div>
							<div className="flex items-center gap-1 font-semibold">
								саша <span className="bg-yellow-400 text-black text-[10px] px-1 rounded-sm flex items-center h-4">JS</span>
							</div>
