...
export default function Avatar({
    ...
    return (
      <div className="chat-avatar avatar">
        <div className="size-10 rounded-full">
          {iconUrl ? (
            <img src={iconUrl} alt={initials} />
          ) : (
            <span className={`${IconsTextSizes[size]} uppercase`}>
              {initials}
            </span>
          )}
        </div>
  ...
  return (
    <div className={`avatar ${initials && 'avatar-placeholder'}`}>
      <div
        className={`${sizes[size]} ${groups[group]} ${badges[badge]} ${variants[variant]} ${shapes[shape]} ${masks[mask]}`}
      >
        {iconUrl ? (
          <img src={iconUrl} alt={initials} />
        ) : (
